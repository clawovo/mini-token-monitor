...
test('source-mode release checks use the public GitHub page instead of the REST API', () => {
  assert.equal(RELEASES_LATEST_URL, 'https://github.com/clawovo/mini-token-monitor/releases/latest');
});
...
test('appUpdateInstallSupport only enables packaged auto-updatable targets', () => {
  ...
  assert.deepEqual(appUpdateInstallSupport({ isPackaged: true, platform: 'linux', env: { APPIMAGE: '/tmp/Mini Token Monitor.AppImage' } }), { supported: true, reason: '' });
});
...
test('parseLatestReleasePayload returns normalized object for valid payload', () => {
  const result = parseLatestReleasePayload({
    tag_name: 'v0.1.3',
    name: 'Mini Token Monitor 0.1.3',
    html_url: 'https://github.com/clawovo/mini-token-monitor/releases/tag/v0.1.3',
...
`
  ...
  assert.deepEqual(result, {
    ...
    tag: 'v0.1.3',
    name: 'Mini Token Monitor 0.1.3',
    htmlUrl: 'https://github.com/clawovo/mini-token-monitor/releases/tag/v0.1.3',
...
test('latestFromUpdaterInfo normalizes provider metadata and release notes', () => {
  assert.deepEqual(latestFromUpdaterInfo({
    version: '0.40.0',
    tag: 'v0.40.0',
    releaseName: 'Mini Token Monitor 0.40.0',
    ...
    version: '0.40.0',
    tag: 'v0.40.0',
    name: 'Mini Token Monitor 0.40.0',
    htmlUrl: 'https://github.com/clawovo/mini-token-monitor/releases/tag/v0.40.0',
