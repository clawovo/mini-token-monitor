...
function apiFetch(keyBodies, creditsBodies = {}) {
  return async (url, init) => {
    assert.equal(init.headers['HTTP-Referer'], 'https://github.com/clawovo/mini-token-monitor');
    assert.equal(init.headers['X-OpenRouter-Title'], 'Mini Token Monitor');
...
test('openrouterToken prefers explicit, then Mini Token Monitor env, then standard env', () => {
