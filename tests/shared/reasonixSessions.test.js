...
function localDeviceView(device) {
  const displayStats = aggregateDevices([device], 0);
  if (device.nativeSessions) displayStats.nativeSessions = device.nativeSessions;
  if (device.nativeProjects) displayStats.nativeProjects = device.nativeProjects;
  const activity = pickRecentUsageActivity({
    periods: device.periods || {
      today: device.today,
      month: device.month,
      allTime: device.allTime
    },
    nativeSessions: device.nativeSessions
  });
  if (activity) displayStats.localRecentUsageActivity = activity;
  return displayStats;
}
...
test('Reasonix telemetry cost is USD-only and never contributes generic project cost', () => {
  ...
  const make = (id, usage) => {
    ...
    return readReasonixNativeSession(paths.metaPath, paths.telemetryPath, {
      projectIdentity: () => ({ projectId: 'opaque', projectLabel: 'Mini Token Monitor' })
    });
  };
...
test('Reasonix native cache discovers a session root created after an empty scan', () => {
  ...
  const projectIdentityForTest = (workspace) => {
    ...
    return { projectId: 'token-monitor-project', projectLabel: 'Mini Token Monitor' };
  };
  ...
  assert.equal(session.projectLabel, 'Mini Token Monitor');
  assert.equal(view.projects.today['mini token monitor'].tokens, 321);
  assert.deepEqual(view.projects.today['mini token monitor'].sessionIds, ['reasonix:created-after-empty']);
  assert.equal(cache.isDirty(), false);
  assert.equal(JSON.stringify(view).includes(stateHome), false);
  assert.equal(JSON.stringify(view).includes(workspaceRoot), false);
...
test('Reasonix native view stays outside aggregate, history, archive and sync payloads', async () => {
  ...
  const displayStats = localDeviceView({
    deviceId: 'native-test',
...
test('Reasonix native rows use the common session formatter and merge project attribution', () => {
  const session = {
    ...
    turns: 2,
    projectLabel: 'Mini Token Monitor',
  ...
  const projects = projectRowsForPeriod({ projects: {} }, {
    nativeProjects: {
      'mini token monitor': { label: 'Mini Token Monitor', tokens: 140, costUsd: 999, clients: { reasonix: 140 } }
    },
    clientLabels: { reasonix: 'Reasonix' },
    clientColors: { reasonix: '#4d6bfe' }
  });
