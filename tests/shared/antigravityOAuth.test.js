...
test('parseClientFromText pairs the trailing Antigravity client when a module embeds several', () => {
  const firstClient = fixtureOAuthClient('111', 'a');
  const lastClient = fixtureOAuthClient('222', 'b');
  const client = antigravityOAuth.parseClientFromText(`
    vs/platform/cloudCode/common/oauthClient.js
    ${firstClient.clientId}
    ${firstClient.clientSecret}
    ${lastClient.clientId}
  `);
  assert.deepEqual(client, { clientId: lastClient.clientId, clientSecret: firstClient.clientSecret });
});

test('parseClientFromText requires complete OAuth credential tokens', () => {
  const incomplete = fixtureOAuthClient('111', 'a');
  const client = antigravityOAuth.parseClientFromText(`
    ${incomplete.clientId}.example.test
    ${incomplete.clientSecret}suffix
  `);
  assert.equal(client, null);
});

test('discoverOAuthClient reports no client when nothing is installed or configured', () => {
...
test('discoverOAuthClient keeps an explicit OAuth client override', () => {
  const client = antigravityOAuth.discoverOAuthClient({
    env: {
