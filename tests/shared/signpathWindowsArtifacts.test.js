...
const APPLICATION = 'Mini Token Monitor.exe';
const INSTALLER = `Mini-Token-Monitor-Setup-${VERSION}.exe`;
const PORTABLE = `Mini-Token-Monitor-${VERSION}.exe`;
...
test('SignPath configurations restrict every signed PE to the release product metadata', () => {
  ...
  assert.deepEqual(openingTagAttributes(artifactXml, 'pe-file'), [
    {
      path: 'installer/Mini-Token-Monitor-Setup-${version}.exe',
      'product-name': pkg.productName,
      'product-version': '${version}'
    },
    {
      path: 'portable/Mini-Token-Monitor-${version}.exe',
      'product-name': pkg.productName,
      'product-version': '${version}'
    }
  ]);
...
function makeFixture(t) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: 'token-monitor',
      version: VERSION,
      productName: 'Mini Token Monitor',
      build: {
        win: {
          verifyUpdateCodeSignature: true,
          signtoolOptions: { publisherName: 'SignPath Foundation' }
        },
        nsis: { artifactName: 'Mini-Token-Monitor-Setup-${version}.${ext}' },
        portable: { artifactName: 'Mini-Token-Monitor-${version}.${ext}' },
...
test('expectedWindowsArtifacts rejects unsafe output names and output-parameter versions', (t) => {
  ...
  pkg.version = VERSION;
  pkg.build.portable.artifactName = '..\\Mini-Token-Monitor-${version}.${ext}';
  fs.writeFileSync(fixture.packageJsonPath, JSON.stringify(pkg));
...
test('expectedWindowsApplication rejects an unsafe product name', (t) => {
  const fixture = makeFixture(t);
  const pkg = JSON.parse(fs.readFileSync(fixture.packageJsonPath, 'utf8'));
  pkg.productName = '..\\Mini Token Monitor';
  fs.writeFileSync(fixture.packageJsonPath, JSON.stringify(pkg));
