...
test('settings page uses collapsible icon sections with summaries', () => {
  ...
  // The Window section is gone; its tray settings moved into General.
  assert.doesNotMatch(html, /data-settings-section="window"/);
  assert.doesNotMatch(html, /windowSettingsDetails/);
...
test('main section holds views; appearance is its own section; tray settings live in general', () => {
  const html = readRendererFile('index.html');
  ...
  // The Window section is removed; its tray settings moved into General, under
  // the Startup group. The app is menu-bar only, so window behavior, the global
  // shortcut, and the floating bubble are no longer offered.
  assert.doesNotMatch(html, /settings-window-section-group/);
  assert.doesNotMatch(html, /settings-window-group/);
  assert.doesNotMatch(html, /id="windowBehaviorInput"/);
  assert.doesNotMatch(html, /id="windowToggleShortcutValue"/);
  assert.doesNotMatch(html, /id="floatingBubbleInput"/);

  const general = html.slice(
  ...
  assert.notEqual(general, '', 'general section should exist');
  const startupIndex = general.indexOf('id="startupGroup"');
  const presenceIndex = general.indexOf('settings-presence-group');
  assert.ok(startupIndex >= 0, 'startup group should be present');
  assert.ok(presenceIndex > startupIndex, 'tray settings should follow startup in general');

  const presenceGroup = general.slice(presenceIndex);
  assert.match(presenceGroup, /id="trayContentInput"/);
  assert.match(presenceGroup, /id="showTrayProviderBadgeInput"/);
  assert.match(presenceGroup, /settings\.display\.menuBarNote/);
  assert.equal((presenceGroup.match(/value="limitsAllSessions"/g) || []).length, 1);
  assert.doesNotMatch(presenceGroup, /id="showTrayIconInput"/);
  assert.doesNotMatch(presenceGroup, /id="trayModeInput"/);
  assert.doesNotMatch(presenceGroup, /id="hideAppIconInput"/);
  assert.doesNotMatch(presenceGroup, /id="trayOptions"/);
  assert.doesNotMatch(presenceGroup, /id="hideAppIconOptions"/);
});
