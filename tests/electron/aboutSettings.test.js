...
test('About uses runtime version and allowlisted Mini Token Monitor links', () => {
  const app = read('app.js');

  assert.match(app, /aboutVersion\.textContent = state\.appInfo\?\.version \? `v\$\{state\.appInfo\.version\}` : '—'/);
  assert.match(app, /TOKEN_MONITOR_REPOSITORY_URL = 'https:\/\/github\.com\/clawovo\/mini-token-monitor'/);
