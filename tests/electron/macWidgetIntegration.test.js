...
test('Widget ownership advances producer lifetime only for mode transitions', () => {
  assert.match(
    mainSource,
    /function startMode\(\) \{\s*advanceMacWidgetProducerAndSourceEpoch\(\);\s*stopLocalCollector\(\);\s*startLocalCollector\(\);/
  );
  assert.match(
    mainSource,
...
test('macOS Widget packaging keeps the canonical Mini Token Monitor app identity', () => {
  assert.equal(packageJson.scripts['mac:local'], undefined);
  assert.equal(packageJson.scripts['mac:local:open'], undefined);
  assert.equal(packageJson.productName, 'Mini Token Monitor');
  assert.equal(packageJson.build.productName, 'Mini Token Monitor');
});
