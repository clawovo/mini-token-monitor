...
// The single remaining widget collector mode must take its usage options from
// electronUsageConfig, or it quietly stops honouring the settings below.
function assertLocalCollectorUsesUsageConfig() {
  const collector = functionSource(main, 'function startLocalCollector()');
  assert.match(collector, /const usageOptions = electronUsageConfig\('collector'\);/);
  assert.match(collector, /^\s+usageOptions,$/m);
}

test('the widget collector follows the retained-session setting for daily history', () => {
  assert.match(main, /function electronUsageConfig/);
  assert.match(main, /usageConfigFromSettings\(settings, \{/);
  assertLocalCollectorUsesUsageConfig();
});

test('the widget collector yields daily-history writes to an external agent', () => {
