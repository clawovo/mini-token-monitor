...
test('autostartSupported requires linux and a running AppImage', () => {
  assert.equal(autostartSupported({ platform: 'linux', env: { APPIMAGE: '/opt/Mini Token Monitor.AppImage' } }), true);
...
test('desktopFileContents produces a desktop entry pointing at the AppImage', () => {
  const contents = desktopFileContents('/opt/apps/Mini Token Monitor.AppImage');
  assert.match(contents, /^\[Desktop Entry\]\n/);
  assert.match(contents, /\nType=Application\n/);
  assert.match(contents, /\nName=Mini Token Monitor\n/);
  assert.match(contents, /\nExec="\/opt\/apps\/Mini Token Monitor\.AppImage"\n/);
...
test('desktopFileContents escapes literal percent signs in Exec arguments', () => {
  const contents = desktopFileContents('/opt/apps/Mini Token Monitor 100%.AppImage');
  const execLine = contents.split('\n').find((line) => line.startsWith('Exec='));
  assert.equal(execLine, 'Exec="/opt/apps/Mini Token Monitor 100%%.AppImage"');
});

test('setAutostartEnabled(true) writes the desktop file, creating the autostart dir', () => {
  const configHome = tmpConfigHome();
  const env = { XDG_CONFIG_HOME: configHome, APPIMAGE: '/opt/Mini Token Monitor.AppImage' };
  ...
  assert.match(written, /Exec="\/opt\/Mini Token Monitor\.AppImage"/);
});

test('isAutostartEnabled requires the desktop file to target the current AppImage', () => {
  const configHome = tmpConfigHome();
  const env = { XDG_CONFIG_HOME: configHome, APPIMAGE: '/opt/Mini Token Monitor.AppImage' };
  const staleEnv = { XDG_CONFIG_HOME: configHome, APPIMAGE: '/old/Mini Token Monitor.AppImage' };
