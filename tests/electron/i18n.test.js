...
test('applyTranslations updates text, title, aria-label, placeholders, and document lang', () => {
  const title = fakeElement({ i18n: 'settings.startup.title' });
  const button = fakeElement({ i18nTitle: 'settings.appUpdate.check' });
  const dismiss = fakeElement({ i18nAriaLabel: 'settings.appUpdate.dismiss' });
  const input = fakeElement({ i18nPlaceholder: 'settings.customPricing.modelPlaceholder' });
  const paste = fakeElement({ i18nTitle: 'settings.appUpdate.viewRelease', i18nAriaLabel: 'settings.appUpdate.viewRelease' });
  ...
  assert.equal(title.textContent, '啟動');
  assert.equal(button.title, '檢查更新');
  assert.equal(paste.title, '查看 release');
  assert.equal(paste.getAttribute('aria-label'), '查看 release');
