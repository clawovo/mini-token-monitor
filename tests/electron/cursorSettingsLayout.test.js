...
test('Cursor removal is available only for accounts added manually in Mini Token Monitor', () => {
...
test('settingsForRenderer strips provider cookies before they reach the renderer', () => {
  ...
  assert.match(body, /credentialSettingsForRenderer\(settings, \{\s*expose: \[\]\s*\}\)/);
...
test('main collectors share one live GUI limit credential resolver in every widget mode', () => {
  ...
  const collectors = [
    functionBody(main, 'startLocalCollector', 'showPopover')
  ];
