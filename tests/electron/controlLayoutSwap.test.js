...
test('window-actions and tabs fade out after a short leave grace delay, in instantly', () => {
  ...
  const reveal = cssRule(css, '.actions-hotspot:hover ~ .window-actions, .window-actions:hover, .window-actions:focus-within');
...
test('Refresh discloses to the left of Settings on hover and keyboard focus', () => {
  ...
  assert.match(css, /\.shell\.settings-open \.utility-actions \.refresh-button \{\s*display: none;/, 'opening Settings hides refresh');
...
test('tray mode removes title-bar hover controls, and Settings hides them too', () => {
  const app = readRendererFile('app.js');
  const css = readRendererFile('styles.css');
  ...
  // The Settings page is a focused surface: no window controls, no hover
  // hotspot, and no refresh button beside the gear.
  const settingsHidden = cssRule(css, '.shell.settings-open .window-actions,\n.shell.settings-open .actions-hotspot,\n.shell.settings-open .utility-actions .refresh-button');
  assert.equal(declaration(settingsHidden, 'display'), 'none');
});

test('the title bar is not a window drag handle', () => {
  const css = readRendererFile('styles.css');
  const titlebar = cssRule(css, '.titlebar');
  assert.match(titlebar, /-webkit-app-region:\s*no-drag;/, 'the menu-bar popover must not be draggable by its top bar');
  assert.doesNotMatch(titlebar, /-webkit-app-region:\s*drag;/);
});
