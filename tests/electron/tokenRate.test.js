...
test('the live footer rate is opt-in, accessible, and shares the persisted mode', () => {
  ...
  assert.match(app, /selectLiveTokenRatePeriods\([\s\S]*stats,[\s\S]*state\.settings\?\.deviceId,[\s\S]*''[\s\S]*effectiveLiveTokenRateScope\(\)[\s\S]*\)/);
  assert.match(app, /return state\.settings\?\.liveTokenRateScope === 'all' \? 'all' : 'device';/);
