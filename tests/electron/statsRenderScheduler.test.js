...
test('leaving Settings no longer needs a catch-up repaint', () => {
  ...
  const settingsToggle = app.slice(
    ...
    app.indexOf("els.appTitleMark?.addEventListener('pointerdown'")
  );
  const trayView = app.slice(
    app.indexOf('function openViewFromTray('),
    app.indexOf('\nconst HOME_HISTORY_MAX_RETRIES')
  ...
  // Opening Settings expands the General section by default, then syncs the form;
  // nothing repaints the main surface, so closing needs no catch-up.
  assert.match(settingsToggle, /if \(settingsOpen\) \{[\s\S]*?setSettingsSectionExpanded\('general', true\);\s+syncSettingsForm\(\);\s+\} else \{/);
...
test('stats-derived Settings rows refresh behind an open panel', () => {
  ...
  const archiveBody = app.slice(
    ...
    app.indexOf('function syncSettingsForm()')
  );
