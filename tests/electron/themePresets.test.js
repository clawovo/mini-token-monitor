...
test('the system preset is first and equals the documented defaults', () => {
  // System stores no overrides and is resolved by the renderer to Obsidian
  // (dark OS) or Porcelain (light OS); its colours here are only the merge base.
  assert.equal(THEME_PRESETS[0].id, 'system');
  assert.deepEqual(THEME_PRESETS[0].colors, DEFAULT_THEME);
  assert.equal(SYSTEM_THEME_ID, 'system');
  assert.equal(SYSTEM_DARK_THEME_ID, 'obsidian');
  assert.equal(SYSTEM_LIGHT_THEME_ID, 'porcelain');
});

test('presetOverrides stores only the keys that differ from the default', () => {
  // System equals the merge base, so its stored form is empty.
  assert.deepEqual(presetOverrides(SYSTEM_THEME_ID), {});
  const obsidian = presetOverrides('obsidian');
  assert.deepEqual(mergeThemeColors(obsidian), THEME_PRESETS.find((p) => p.id === 'obsidian').colors);
  const porcelain = presetOverrides('porcelain');
  assert.deepEqual(mergeThemeColors(porcelain), THEME_PRESETS.find((p) => p.id === 'porcelain').colors);
  assert.deepEqual(presetOverrides('missing'), {});
});

test('systemThemeOverrides resolves to obsidian on a dark OS and porcelain on a light one', () => {
  assert.deepEqual(mergeThemeColors(systemThemeOverrides(true)), THEME_PRESETS.find((p) => p.id === 'obsidian').colors);
  assert.deepEqual(mergeThemeColors(systemThemeOverrides(false)), THEME_PRESETS.find((p) => p.id === 'porcelain').colors);
});
