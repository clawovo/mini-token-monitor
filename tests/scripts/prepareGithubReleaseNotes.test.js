...
test('composeReleaseNotes inserts GitHub notes below the single summary link', () => {
  const template = [
    '# User-facing notes',
    '<details>',
    '<summary><strong>Full Changelog:</strong> <a href="https://github.com/clawovo/mini-token-monitor/compare/v1.0.0...v1.1.0">v1.0.0...v1.1.0</a></summary>',
    '',
    GENERATED_NOTES_MARKER,
    '</details>'
  ].join('\n');
...
test('composeReleaseNotes adds direct commits to What\'s Changed before New Contributors', () => {
  ...
  assert.match(result, /fix\(widget\).*by @direct-author \(\[abcdef1\]\(https:\/\/github\.com\/clawovo\/mini-token-monitor\/commit\/abcdef1234567890abcdef1234567890abcdef12\)\)/);
...
test('fullChangelogRange locks generated notes to the curated compare range', () => {
  assert.deepEqual(
    fullChangelogRange(
      '<summary><strong>Full Changelog:</strong> <a href="https://github.com/clawovo/mini-token-monitor/compare/v0.42.1...v0.43.0">v0.42.1...v0.43.0</a></summary>'
    ),
    { previousTag: 'v0.42.1', currentTag: 'v0.43.0' }
  );
  assert.deepEqual(
    fullChangelogRange(
      '<summary><strong>Full Changelog:</strong> <a href="https://github.com/clawovo/mini-token-monitor/compare/v1.0.0...v1.1.0-beta.1+build.2">v1.0.0...v1.1.0-beta.1+build.2</a></summary>'
    ),
    { previousTag: 'v1.0.0', currentTag: 'v1.1.0-beta.1+build.2' }
  );
  assert.throws(
    () => fullChangelogRange(
      '<summary><strong>Full Changelog:</strong> <a href="https://github.com/clawovo/mini-token-monitor/compare/v1.0.0...v1.1.0">v1.0.0...v1.2.0</a></summary>'
    ),
...
test('fetchGeneratedNotes requests GitHub generated notes for the pushed tag', async () => {
  ...
  assert.equal(request.url, 'https://api.github.com/repos/clawovo/mini-token-monitor/releases/generate-notes');
...
test('fetchGeneratedNotes honors GITHUB_API_URL and fails when its timeout signal fires', async () => {
  await assert.rejects(
    fetchGeneratedNotes({
      ...
      fetchImpl: async (url, options) => {
        assert.equal(url, 'https://github.example/api/v3/repos/clawovo/mini-token-monitor/releases/generate-notes');
...
test('fetchDirectCommits keeps only commits without an associated merged PR', async () => {
  ...
  assert.equal(
    requests[0],
    'https://github.example/api/v3/repos/clawovo/mini-token-monitor/compare/v1.0.0...v1.1.0?per_page=100&page=1'
  );
});
