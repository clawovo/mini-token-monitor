...
Zed appears in Mini Token Monitor in two independent data planes. Keep them separate when changing or debugging the provider.
...
The normalized credential must contain `zed.session`. Mini Token Monitor forwards only the observed Zed billing cookies (`zed.session`, `c15t`, and supported Cloudflare helpers) to `cloud.zed.dev`; unrelated cookies from the copied header are discarded. Provider requests use `credentials: 'omit'` so Electron's ambient cookie jar cannot replace the explicitly managed credential.
...
| Zed field | Mini Token Monitor output |
| --- | --- |
