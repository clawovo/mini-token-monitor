...
Tokscale 4.15.1 reads `~/.unsloth/studio/studio.db`, or `studio.db` directly under `UNSLOTH_STUDIO_HOME` when that override is nonblank. Mini Token Monitor checks for the database itself, not merely the Studio installation directory.
