...
Mini Token Monitor's pinned tokscale build uses the same canonical generic-version matcher. Keeping
...
on the next refresh. DSH validates titles before persistence, so Mini Token Monitor preserves
