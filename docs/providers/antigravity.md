...
Antigravity appears in Mini Token Monitor in two independent data planes. Keep them separate when changing or debugging the provider.

| Data plane | What it measures | Primary runtime | Inputs |
| --- | --- | --- | --- |
| Token/session activity | Local model-token activity attributed to Antigravity | Shared usage collector through `tokscale` | Local Antigravity conversation data and the Mini Token Monitor-managed tokscale cache |
...
Mini Token Monitor recognizes these native Antigravity roots under `~/.gemini/`:
...
The generated `antigravity-cache` directory is deliberately not watched. Mini Token Monitor's own sync writes it, so watching it would create a refresh loop.
...
When managed OAuth accounts exist, Mini Token Monitor fetches enabled OAuth accounts and the local RPC snapshot concurrently. If local RPC returns a trusted email matching an OAuth account, the live local result replaces the remote result for that account.
