...
The current release workflow Authenticode-signs Mini Token Monitor's Windows application executable, installer (`Mini-Token-Monitor-Setup-*.exe`), and portable build (`Mini-Token-Monitor-*.exe`). The expected publisher is **SignPath Foundation**.
...
```powershell
Get-AuthenticodeSignature ".\Mini-Token-Monitor-Setup-<version>.exe", ".\Mini-Token-Monitor-<version>.exe" | Format-List Path, Status, SignerCertificate, TimeStamperCertificate
```

A genuine Mini Token Monitor release should show `Valid` status, SignPath Foundation as the signer, and timestamp information.

After installation, the same check can be run against `Mini Token Monitor.exe` in the application's installation directory.
...
- Committers and reviewers: [@clawovo](https://github.com/clawovo)
- Approvers: [@clawovo](https://github.com/clawovo)

## Privacy

- Privacy policy: [Mini Token Monitor privacy policy](privacy.md)
