...
Mini Token Monitor can export your usage data in **tool-agnostic** formats so you can
...
- **Export data…** — pick a folder; Mini Token Monitor writes the file set below into
  it once, right now.
...
the same token-component semantics as Mini Token Monitor. When older or partial
