...
On Windows, Mini Token Monitor normally scans supported tools inside every running WSL distribution through `\\wsl$` and merges their usage about every five minutes. File-based sources such as Codex JSONL sessions work well with this path.

SQLite-backed tools such as OpenCode, Hermes, and ZCode store current usage in SQLite databases. A Windows process can discover those databases through `\\wsl$` while SQLite still cannot reliably coordinate locks or an active WAL across the WSL 9P boundary. Mini Token Monitor may therefore show the tool under **Settings → Collection → WSL detection** with no usage.
...
```text
WSL headless agent → Windows host hub → Mini Token Monitor widget
```
...
In Mini Token Monitor, open **Settings → Multi-device Sync** and select **Host hub on this device**. Record the hub URL and shared secret.
...
Mini Token Monitor requires Node.js 22.15.0 or newer. Verify Node.js and npm inside WSL before installing; upgrade Node.js first if the reported version is older.

```bash
node --version
npm --version
git clone https://github.com/clawovo/mini-token-monitor.git
cd token-monitor
npm ci
```
...
Confirm that a second device appears in Mini Token Monitor and that the SQLite-backed tool has usage. Then run the continuous agent:

```bash
npm run agent
```

For unattended use, run that command from your normal WSL service manager or login startup. Keep its working directory set to the Mini Token Monitor checkout so `.env` is loaded.
