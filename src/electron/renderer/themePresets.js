...
(function exposeThemePresets(root, factory) {
  ...
  // just the accent. The two explicit themes are a near-black "Obsidian" and a
  // light "Porcelain"; Porcelain relies on the light-mode flip in
  // themeCssVarEntries() so borders/panels stay visible on a pale base.
  //
  // `system` is the default: it stores no overrides and the renderer resolves it
  // to Obsidian on a dark OS and Porcelain on a light one, re-resolving when the
  // OS theme changes. Its `colors` here are only the merge base (the graphite
  // DEFAULT_THEME) and are never shown; the chip dot is painted from the live
  // resolved accent. Keeping the id in this list lets matchingThemePresetId()
  // map an empty override set back to the System chip.
  const SYSTEM_THEME_ID = 'system';
  const SYSTEM_DARK_THEME_ID = 'obsidian';
  const SYSTEM_LIGHT_THEME_ID = 'porcelain';
  const THEME_PRESETS = [
    { id: SYSTEM_THEME_ID, colors: { ...DEFAULT_THEME } },
  ...
  const VENDOR_ORDER = [
    ...
    'droid', 'kimi', 'qwen', 'grok', 'copilot', 'pi', 'zed', 'kilo', 'commandcode', 'micode', 'zcode', 'kiro', 'codebuddy', 'workbuddy', 'proma', 'qodercn', 'reasonix', 'dsh', 'cherrystudio', 'lmstudio', 'unsloth', 'penguin',
  ...
  const VENDOR_LABELS = {
    ...
    lmstudio: 'LM Studio',
    unsloth: 'Unsloth', penguin: 'Penguin Harness',
  ...
  // The override object that reproduces a preset: only the keys that differ from
  // the built-in default, so untouched colours keep tracking DEFAULT_THEME. An
  // empty object is the System theme's stored form (see app.js).
  function presetOverrides(presetId) {
    const preset = THEME_PRESETS.find((p) => p.id === presetId);
    const out = {};
    if (!preset) return out;
    for (const key of INTERFACE_COLOR_KEYS) {
      if (preset.colors[key] !== DEFAULT_THEME[key]) out[key] = preset.colors[key];
    }
    return out;
  }

  // What the System theme resolves to for the current OS appearance.
  function systemThemeOverrides(dark) {
    return presetOverrides(dark ? SYSTEM_DARK_THEME_ID : SYSTEM_LIGHT_THEME_ID);
  }
