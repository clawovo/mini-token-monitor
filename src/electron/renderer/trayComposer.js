...
(function exposeTrayComposer(root, factory) {
  ...
  function createTrayComposer(options) {
    ...
    function styleTitle(style) {
      const fallback = {
        appIcon: 'Mini Token Monitor icon',
    ...
    function barIconEditor(item) {
      ...
      const choices = [{
        value: 'app',
        label: l('trayComposer.icon.app', 'Mini Token Monitor'),
        icon: app?.icon
      }, choiceForRow(0)];
    ...
    function renderItemPopover(id) {
      ...
      if (item.type === 'icon') {
        const iconTypes = [
          { value: 'app', label: l('trayComposer.icon.app', 'Mini Token Monitor'), icon: providerChoices('auto').find((choice) => choice.value === 'auto')?.icon },
    ...
    function render() {
      ...
      if (!editable) {
        ...
        if (preview.generatedSrc) {
          ...
          content.textContent = preview.text || 'M';
        }
