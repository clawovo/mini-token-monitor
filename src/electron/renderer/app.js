...
const clientsWithIcon = new Set([
  'claude', 'codex', 'opencode', 'hermes', 'openclaw', 'cursor', 'antigravity', 'cline', 'droid', 'kimi', 'qwen', 'grok', 'copilot', 'pi', 'zed', 'kilo', 'commandcode', 'micode', 'zcode', 'kiro', 'codebuddy', 'workbuddy', 'proma', 'qodercn', 'reasonix', 'dsh', 'cherrystudio', 'lmstudio', 'unsloth', 'penguin',
...
const TOKEN_MONITOR_REPOSITORY_URL = 'https://github.com/clawovo/mini-token-monitor';
...
const SETTINGS_SECTION_IDS = ['general', 'main', 'appearance', 'tools', 'limits', 'subscriptions'];
...
function effectiveLiveTokenRateScope() {
  return state.settings?.liveTokenRateScope === 'all' ? 'all' : 'device';
}
...
function openSettingsPanel({ expandGeneral = false } = {}) {
  if (!els.settingsPanel) return;
  if (state.viewSwitcherOpen) setViewSwitcherOpen(false);
  els.settingsPanel.classList.remove('hidden');
  els.shell.classList.add('settings-open');
  // Settings is a whole-page destination and opens on the General section.
  if (expandGeneral) setSettingsSectionExpanded('general', true);
  syncSettingsForm();
  ensureServiceStatusTicker();
  els.shell.style.transform = 'translateZ(0)';
...
function applyAppearanceSettings(settings) {
  ...
  if (settings && 'themeColors' in settings) applyThemeColors(effectiveThemeOverrides());
...
// The System theme stores no overrides and resolves to a dark or light preset
// from the OS appearance. `themeFollowsSystem()` therefore means "no stored
// overrides"; any explicit preset or custom edit stores overrides and stops
// tracking the OS.
function themeFollowsSystem() {
  return Object.keys(currentThemeOverrides()).length === 0;
}

// The overrides actually applied: the live system palette while following the
// OS, otherwise whatever the user stored.
function effectiveThemeOverrides() {
  return themeFollowsSystem()
    ? themePresetsApi.systemThemeOverrides(state.systemDarkUi === true)
    : currentThemeOverrides();
}

// Returns the preset id whose colours exactly match the resolved palette, or
// null when the palette is a custom mix. An empty override set matches the
// System preset (whose colours are the merge base).
...
function renderThemePresetChips() {
  ...
  const activeId = matchingThemePresetId(currentThemeOverrides());
  ...
  for (const preset of themePresetsApi.THEME_PRESETS) {
    ...
    // The System chip previews the live OS palette, not its static merge base.
    dot.style.background = preset.id === themePresetsApi.SYSTEM_THEME_ID
      ? themePresetsApi.mergeThemeColors(themePresetsApi.systemThemeOverrides(state.systemDarkUi === true)).accent
...
function previewThemeColor(key, value) {
  ...
  // Editing starts from what is on screen, so a change made while following the
  // OS keeps the rest of the live system palette instead of reverting to graphite.
  const next = { ...effectiveThemeOverrides(), [key]: themePresetsApi.normalizeHex(value) };
...
async function selectThemePreset(presetId) {
  if (!themePresetsApi.THEME_PRESETS.some((p) => p.id === presetId)) return;
  // System stores no overrides (it resolves to the live OS palette); the others
  // store only the keys that differ from the built-in default, so untouched
  // colours keep tracking DEFAULT_THEME.
  await commitThemeColors(
    presetId === themePresetsApi.SYSTEM_THEME_ID ? {} : themePresetsApi.presetOverrides(presetId)
  );
}

async function commitThemeColors(overrides) {
  state.settings.themeColors = overrides;
  // Resolve System (empty overrides) to the live OS palette before applying.
  applyThemeColors(effectiveThemeOverrides());
...
function renderFloatingBubbleContent() {
  ...
  if (window.TokenMonitorTrayText.isGeneratedTrayIconMode(mode)) {
    ...
    el.textContent = (state.stats && window.TokenMonitorTrayText.formatTrayText(state.stats, mode, currentCurrency(), compactTokenDisplayOptions())) || 'M';
  } else if (mode === 'icon') {
    floatingBubbleRenderedBitmapHeight = null;
    el.classList.remove('bars');
    el.textContent = 'M';
...
els.settingsBackButton?.addEventListener('click', (event) => {
  if (state.viewSwitcherOpen) setViewSwitcherOpen(false);
  els.settingsPanel?.classList.add('hidden');
  els.shell.classList.remove('settings-open');
  if (!renderBreakdownChange('home')) return;
...
async function init() {
  ...
  const applySystemUiTheme = (dark) => {
    ...
    // The System interface theme resolves to Obsidian or Porcelain from the OS
    // appearance, so a flip re-resolves it (settings load first: before then
    // there is nothing stored to keep in sync).
    if (state.settings && themeFollowsSystem()) {
      applyThemeColors(effectiveThemeOverrides());
      buildAppearanceColorControls();
    }
...
els.settingsButton.addEventListener('click', (event) => {
  ...
  // Settings is a top-level page with its own Back to Home button. Closing it
  // needs no catch-up repaint — only the panel's own DOM has to be caught up
  // when it opens, because its renderers idle while it is closed.
  if (settingsOpen) {
    // Every fresh open lands on the General section.
    setSettingsSectionExpanded('general', true);
    syncSettingsForm();
  } else {
    resetSettingsListSearch();
  }
...
els.showLiveTokenRateInput.addEventListener('change', async () => {
  ...
  els.liveTokenRateScopeRow?.classList.toggle('hidden', !state.settings.showLiveTokenRate);
...
window.tokenMonitor.onOpenSettings?.(() => openSettingsPanel({ expandGeneral: true }));
...
function drawTrayFallbackMark(ctx, value, x, y, size, color) {
  ...
  ctx.textBaseline = 'middle';
  ctx.fillText(value === 'app' ? 'M' : String(value || '?').slice(0, 1).toUpperCase(), x + size / 2, y + size / 2 + 1);
  ctx.restore();
}
...
function renderOpenCodeProfiles() {
  ...
  api.getProfiles().then(({ profiles, hasEnvVar, hasAmbientKey, ambientEnabled = true }) => {
    ...
    // quota. It has no toggle or delete because Mini Token Monitor does not own that
