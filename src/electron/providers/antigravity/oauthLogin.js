...
function callbackPage(ok) {
  ...
  const detail = ok
    ? 'Return to Mini Token Monitor to finish connecting this account.'
    : 'Return to Mini Token Monitor for details.';
...
async function runAntigravityOAuthLogin(options = {}) {
  ...
  if (!client) {
    throw loginError(
      ...
      'Antigravity OAuth is unavailable in this build. Update Mini Token Monitor or configure ANTIGRAVITY_OAUTH_CLIENT_ID and ANTIGRAVITY_OAUTH_CLIENT_SECRET.'
    );
  }
