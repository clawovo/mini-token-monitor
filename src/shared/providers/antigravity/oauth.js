...
function discoverOAuthClient(options = {}) {
  ...
  // No bundled fallback: Antigravity's desktop OAuth client is a third-party
  // credential, so it is never shipped in the repository. A machine with neither
  // an installed Antigravity app to read it from nor an explicit override keeps
  // the client unavailable and surfaces that instead of signing in with a
  // credential this project does not own.
  return null;
}
